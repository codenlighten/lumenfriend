...
export async function handleStatsCommand(userId, session, stats) {
  ...
  const responseTypes = stats.responseTypes || { response: 0, code: 0, terminalCommand: 0 };
  const responseTypeStr = `  💬 Conversations: ${responseTypes.response || 0}
  💻 Code Generated: ${responseTypes.code || 0}
  🔧 Commands: ${responseTypes.terminalCommand || 0}`;
