...
export const AVAILABLE_FUNCTIONS = [
  ...
  },
  {
    ...
    description: "Generate and execute terminal commands to examine code, run tests, create files, or interact with the filesystem. ALWAYS requires user approval before execution. Use when user asks to read files, check code, run tests, examine the codebase, or perform system operations.",
    parameters: {
      type: "object",
      properties: {
        task: {
          type: "string",
          description: "Clear description of what you want to accomplish, e.g. 'Read the contents of lib/statsTracker.js' or 'Count total lines of JavaScript code' or 'Run the test suite'"
        }
      },
      required: ["task"]
    }
  ...
  try {
    switch (functionName) {
      ...
      case "execute_terminal_command": {
        // Generate command via terminal-agent API
        const response = await fetch(`${baseUrl}/api/terminal-agent`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ query: parameters.task })
        });
        const result = await response.json();
        ...
        return {
          ...
          explanation: result.data?.result?.explanation || parameters.task,
          data: result.data
        };
      }
 ...
 */
function formatCommandApprovalRequest(command, explanation) {
  return `**🖥️ Terminal Command Request**
