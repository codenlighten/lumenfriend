...
/**
 * Detect topics from message content using keyword matching
 */
function detectTopics(userMessage, responseMessage) {
  const keywords = {
...
export async function recordInteraction(userId, stats, userMessage, responseMessage, responseTime = 0, detectedTopics = []) {
  ...
  const topics = detectTopics(userMessage, responseMessage)

  // Track topics (combine detected and provided, remove duplicates)
  const allTopics = [...new Set([...topics, ...detectedTopics])];
